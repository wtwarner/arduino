 ...
 */
void Yin_cumulativeMeanNormalizedDifference(Yin16 *yin){
  int16_t tau;
  float runningSum = 0;
  yin->yinBuffer[0] = 1;
 ...
 */
void Yin_init(Yin16 *yin, int16_t bufferSize, float threshold, int sampling_rate){
 ...
 */
float Yin_getPitch(Yin16 *yin, int16_t* buffer){
  ...
  /* Step 5: Interpolate the shift value (tau) to improve the pitch estimate. */
  if(tauEstimate != -1){
    pitchInHertz = yin->sampling_rate / Yin_parabolicInterpolation(yin, tauEstimate);
  }
 ...
 */
float Yin_getProbability(Yin16 *yin){
